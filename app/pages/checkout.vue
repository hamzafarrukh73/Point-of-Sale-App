                            ...
                            <div class="flex flex-col gap-3 h-full w-full justify-between">
                                <div class="flex flex-col gap-3 h-4/10">
                                    ...
                                    <div class="flex flex-col overflow-auto w-full grow">
                                        <div class="flex flex-col grow gap-3 w-full justify-center" v-if="itemsCount === 0">
                                            <p class="text-muted text-center">No items in cart.</p>
                                        </div>
                                        <div v-else="itemsCount > 0" v-for="item in sale.items" :key="item.product_id" class="flex flex-col  w-full" >
                                            <div class="flex w-full justify-between pe-3">
                                                <p>{{ item.name_at_sale }}</p>
                                                <div class="flex gap-2 items-center justify-end w-fit ">
                                                    <UInputNumber v-model="item.quantity" min="1" :max="inventory.find(inv => inv.id === item.product_id)?.stock_qty" class="w-30" />
                                                    <UButton icon="i-fa-trash" color="error" size="md" @click="removeFromCart(item.product_id)" />
                                                </div>
                                            </div>                                        
                                            <p class="text-muted">${{ (item.price_at_sale * item.quantity).toFixed(2) }}</p>
                                        </div>
                                    </div>
                                    <USeparator orientation="horizontal" />
                                </div>
                        ...
                        <template #footer>
                            <div class="flex flex-row w-full gap-3">
                                ...
                                <div class="flex flex-row gap-3">
                                    ...
                                    <UButton label="Checkout" color="success" size="lg" class="w-fit" @click="checkout" :disabled="itemsCount === 0" />
                                </div>
                            </div>
                        </template>
                    </USlideover>
                    ...
                    <template #footer>
                        <UContainer class="w-full m-0! p-0! flex justify-between items-center">
                            ...
                            <UContainer class="flex flex-row col-span-1 gap-3 p-0! m-0! justify-end">              
                                ...
                                <UButton label="Remove" @click="removeFromCart(product.id)" v-if="product.stock_qty>0" color="error" />
                                <UButton label="No Stock" color="error" variant="outline" v-else="product.stock_qty<=0" />
                            </UContainer>
                        </UContainer>
                    </template>
                </UCard>
