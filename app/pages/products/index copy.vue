        ...
        <div>
            <div class="actions">
                ...
                <div class="flex justify-end w-3/10 gap-2">
                    <!-- <manage_categories /> -->
                    <UButton label="Categories" to="products/categories" size="sm" class="w-1/2 place-content-center" />
        ...
        <div class="container-toolkit">
            ...
            <div class="flex w-1/4 justify-end gap-2">
                <UForm class="flex-grow-1">
                    <USelectMenu placeholder="Sort by" :items="productsSortMenu" @change="sortProducts"
                        v-model="productsOrder" arrow variant="soft" color="primary" size="md" class="w-full"
                ...
                <USlideover title="Filters">
                        ...
                        <div class="flex h-full w-full">
                            <UForm class='flex-col gap-3 w-full flex-grow-1'>
                                <UFormField label="Category">
                                    <USelectMenu placeholder="Category" arrow :items="categoriesMenu"
                                        v-model="productsFilters.category" class="w-1/2 m-2"
                                        :ui="{ content: 'min-w-fit' }" />
                                </UFormField>
                                <UFormField label="Min Price">
                                    <UInputNumber placeholder="Min Price" v-model="productsFilters.min_price" min="0"
                                        class="w-1/2 m-2" />
                                </UFormField>
                                <UFormField label="Max Price">
                                    <UInputNumber placeholder="Max Price" v-model="productsFilters.max_price" min="0"
                                        class="w-1/2 m-2" />
                                </UFormField>
                            </UForm>
                        </div>
                    ...
                    <template #footer>
                        <div class="flex w-full justify-end">
                            <UButton label="Apply Filters" size="lg" class="flex-shrink-0" @click="filterProducts" />
                        </div>
                    </template>
                </USlideover>
            </div>
